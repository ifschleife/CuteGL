...
void OpenGLWindow::initializeGL()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, m_sphere.positions.size() * sizeof(m_sphere.positions[0]), m_sphere.positions.data(),
                 GL_STATIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);

    glGenBuffers(1, &m_sphere.vbos.index);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_sphere.vbos.index);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_sphere.indices.size() * sizeof(m_sphere.indices[0]),
                 m_sphere.indices.data(), GL_STATIC_DRAW);
...
void OpenGLWindow::paintGL()
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m_sphere.vbos.pos);
    glVertexAttribPointer(m_sphere_shader->attributeLocation("position"), 3, GL_FLOAT, GL_FALSE, 0,
                          nullptr); // nullptr == uses currently bound buffer e.g. m_vbo_vertices
